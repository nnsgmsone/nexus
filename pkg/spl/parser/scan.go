...
import (
	...
	"github.com/nnsgmsone/nexus/pkg/spl/lex"
...
func (s *scanner) scan(lval *splSymType) {
	lval.id = 0
	...
	case identQuote:
		// "[^"]"
		if s.scanString(lval, ch, true /* allowEscapes */, true /* requireUTF8 */) {
			lval.id = SCONST
			lval.union.val = &tree.Value{
				Value: constant.MakeString(lval.str),
			}
		}
		return
	case backQuote:
...
func (s *scanner) skipWhitespace(lval *splSymType, allowComments bool) (newline, ok bool) {
	newline = false
	for {
		ch := s.peek()
		if ch == '\n' {
...
func (s *scanner) scanComment(lval *splSymType) (present, ok bool) {
	start := s.pos
	ch := s.peek()
...
func (s *scanner) scanIdent(lval *splSymType) {
	s.pos--
	start := s.pos
	isASCII := true
	isLower := true
...
func (s *scanner) scanNumber(lval *splSymType, ch int) {
	start := s.pos - 1
	isHex := false
	hasDecimal := ch == '.'
	hasExponent := false
...
// scanHexString().
func (s *scanner) scanString(lval *splSymType, ch int, allowEscapes, requireUTF8 bool) bool {
	buf := s.buffer()
...
outer:
	for {
		...
		case ch:
			...
			// SPL allows joining adjacent strings separated by whitespace
...
func EndsInSemicolon(spl string) bool {
	lastTok, ok := LastLexicalToken(spl)
	return ok && lastTok == ';'
}
