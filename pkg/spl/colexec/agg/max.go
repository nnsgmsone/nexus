...
func (a *max[T]) Load(vec *vector.Vector) error {
	var err error
	var data []byte
...
func (a *max[T]) Save() (*vector.Vector, error) {
...
func (a *max[T]) Specialize(proc *process.Process) error {
	var err error
...
func (a *max[T]) BulkFill(grp uint32, vecs []*vector.Vector) error {
...
func (a *max[T]) Fill(grps, sels []uint32, vecs []*vector.Vector) error {
	vs := vector.GetColumnValue[T](vecs[0])
	for i, g := range grps {
...
func (a *max[T]) Merge(o Agg, agrps, bgrps []uint32) error {
	b := o.(*max[T])
	for i, g := range agrps {
...
func (a *max[T]) Eval() (*vector.Vector, error) {
...
func (a *boolMax) Load(vec *vector.Vector) error {
	var err error
	var data []byte
...
func (a *boolMax) Save() (*vector.Vector, error) {
	var buf bytes.Buffer
...
func (a *boolMax) Specialize(proc *process.Process) error {
	var err error
...
func (a *boolMax) BulkFill(grp uint32, vecs []*vector.Vector) error {
...
func (a *boolMax) Fill(grps, sels []uint32, vecs []*vector.Vector) error {
	vs := vector.GetColumnValue[bool](vecs[0])
	for i, g := range grps {
		if vecs[0].IsNull(int(sels[i])) {
			continue
...
func (a *boolMax) Merge(o Agg, agrps, bgrps []uint32) error {
	b := o.(*boolMax)
	for i, g := range agrps {
		if a.cnts[g] == 0 || (b.cnts[bgrps[i]] > 0 && (b.vals[bgrps[i]] && !a.vals[g])) {
			a.vals[g] = b.vals[bgrps[i]]
...
func (a *boolMax) Eval() (*vector.Vector, error) {
	if err := a.vec.PreExtend(len(a.vals)); err != nil {
		return nil, err
	}
	a.vec.Reset()
...
func (a *strMax) Load(vec *vector.Vector) error {
	var err error
	var data []byte
...
func (a *strMax) Save() (*vector.Vector, error) {
	var buf bytes.Buffer
...
func (a *strMax) Specialize(proc *process.Process) error {
	var err error
...
func (a *strMax) BulkFill(grp uint32, vecs []*vector.Vector) error {
...
func (a *strMax) Fill(grps, sels []uint32, vecs []*vector.Vector) error {
	vs := vector.GetColumnValue[types.String](vecs[0])
	for i, g := range grps {
		if vecs[0].IsNull(int(sels[i])) {
			continue
...
func (a *strMax) Merge(o Agg, agrps, bgrps []uint32) error {
	b := o.(*strMax)
	for i, g := range agrps {
...
func (a *strMax) Eval() (*vector.Vector, error) {
	if err := a.vec.PreExtend(len(a.vals)); err != nil {
		return nil, err
	}
	a.vec.Reset()
