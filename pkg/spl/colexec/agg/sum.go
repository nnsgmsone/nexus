...
func (a *sum[T]) Load(vec *vector.Vector) error {
	var err error
	var data []byte
...
func (a *sum[T]) Save() (*vector.Vector, error) {
...
func (a *sum[T]) Specialize(proc *process.Process) error {
	var err error
...
func (a *sum[T]) BulkFill(grp uint32, vecs []*vector.Vector) error {
...
func (a *sum[T]) Fill(grps, sels []uint32, vecs []*vector.Vector) error {
	vs := vector.GetColumnValue[T](vecs[0])
	for i, g := range grps {
...
func (a *sum[T]) Merge(o Agg, agrps, bgrps []uint32) error {
	b := o.(*sum[T])
	for i, g := range agrps {
...
func (a *sum[T]) Eval() (*vector.Vector, error) {
	...
	for i, v := range a.vals {
		if a.cnts[i] == 0 {
			if err := vector.Append(a.vec, v, true); err != nil {
				return nil, err
			}
			continue
		}
